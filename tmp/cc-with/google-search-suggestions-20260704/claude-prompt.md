# ClaudeCode Task Handoff

## Role
You are ClaudeCode. Your role is to execute an already-planned implementation task. Do not redefine scope or architecture.

## Task Goal
Add Google-powered realtime search suggestions to the new-tab search box

## Required Execution Order
1. Use `superpowers:using-git-worktrees`.
2. Create an isolated worktree.
3. Perform implementation only inside that worktree.
4. Use `superpowers:subagent-driven-development` inside that worktree.
5. If either workflow is unavailable, stop and report the blocker.
...
## Output Requirements
Return results in this exact plain-text structure:
...
- branch:
- head_commit:
- subagent_workflow_used: superpowers:subagent-driven-development
- summary:
- files_changed:
- checks_run:
- checks_passed:
